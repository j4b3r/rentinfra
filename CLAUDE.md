...
**Cars** (`/admin/cars`): list with image, price, status (including the derived vehicle-status badge — see fleet maintenance below). Add/edit at `/admin/cars/new` and `/admin/cars/[id]`. CarForm covers all fields + pricing + discount tiers + photo upload + maintenance blocks (edit mode only). Photos managed via `CarImageUploader` — drag/drop, set cover, delete.
