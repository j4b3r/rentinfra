...
- `013_payments.sql` — `stripe_session_id`/`amount_paid`/`paid_at`/`refunded_amount` on bookings; `expire_stale_booking_holds()` gains a paid-booking guard
- `012_vehicle_types.sql` — `cars.vehicle_type` (car|motorbike|bicycle), per-type category constraint, `engine_cc`/`frame_size`/`gears`/`helmet_included`, `requires_license`/`min_rider_age`, `price_lists.rate_unit`/`hourly_rate`, `addons.vehicle_type`
- `014_condition_reports.sql` — `booking_condition_photos` (per booking/phase), `booking_damage_claims` (staff-raised, cites photo IDs), private `condition-photos` storage bucket (signed URLs only, unlike public `car-images`)
