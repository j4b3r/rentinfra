...
### Booking Flow
Car detail → `/booking/[carSlug]` → `BookingWizard` (4 steps: Dates, Extras, Details, Review) → POST `/api/bookings` → Confirmation page with reference (CMB-YYYY-NNNNN)
...
### Admin Panel (`/admin`)
Sidebar nav: Dashboard, Bookings, Cars, Addons, Users, Locations, Settings, Translations.
