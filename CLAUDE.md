...
- **Stack**: Next.js 16 (App Router), Tailwind CSS, Supabase (PostgreSQL + Auth + Storage), next-intl (EN/ES), Vitest
...
for visibility, in case a future dependency bump introduces a high-severity finding that isn't
worth failing the build over immediately. `next` and `eslint-config-next` are meant to track
the same major.minor together — `dependabot.yml` deliberately excludes `eslint-config-next`
from automatic bumps so it doesn't drift ahead of `next` on its own; bump both by hand.
**Dependabot auto-merge** (`.github/workflows/dependabot-auto-merge.yml`): patch/minor
dependency bumps merge themselves once CI passes; majors are left open for manual review,
since a major passing CI isn't the same safety guarantee a patch/minor passing CI is (this
caught two real breaks in practice — a linter major that crashed on the config, and a
compiler major Next.js couldn't run yet). **CodeQL** (`.github/workflows/codeql.yml`) does static
...
### Booking Flow
Homepage `SearchPanel` (dates + location) → `/cars?pickup=&dropoff=&location=` → car detail → `/booking/[carSlug]?pickup=&dropoff=&location=` → `BookingWizard` (4 steps: Dates, Extras, Details, Review) → POST `/api/bookings` → Confirmation page with reference (RIF-YYYY-NNNNN)
...
**Signatures** (`SignaturePanel`, mounted on the booking detail page): captures a signature per (booking, role, stage) — role is `client`/`company`, stage is `contract`/`delivery`/`return`, matching the three sign-here areas in the PDF contract — via `react-signature-canvas` into a **private** `signatures` Storage bucket, following the same private-bucket + signed-URL pattern as condition photos and licence documents (`booking_signatures` table, `is_admin()` RLS, 10-min `createSignedUrl()` TTL, never a stored public URL). Re-signing the same role/stage replaces the prior image (`UNIQUE(booking_id, role, stage)`), it doesn't accumulate. The contract PDF route downloads each signature directly via the admin client (not a signed-URL fetch — `@react-pdf/renderer`'s `Image` needs a data URL/buffer) and inlines it as base64, same treatment as the logo/QR; any stage/role without a captured signature falls back to the original blank line, so print-and-sign-by-hand keeps working when e-sign wasn't used.
...
- Booking references auto-generated by DB trigger: `RIF-YYYY-NNNNN`
...
Full public website (home, blog, FAQ, legal pages, EN/ES), auth (Google OAuth + email/password),
booking funnel with an availability system that can't double-book, payments via Stripe (with 3D
Secure), the full admin panel (bookings, fleet with maintenance scheduling, addons, locations,
users, testimonials, settings, business reporting), vehicle handover (condition photos, damage
claims, licence verification, e-signature capture), PDF rental contracts, EV vehicle support,
per-branch fleet, WhatsApp/SMS/email notifications, an outbound OTA/channel-manager feed, a test
suite with CI/CodeQL/Dependabot, and a custom domain on the live demo.

See **[ROADMAP.md](./ROADMAP.md)** for the authoritative, dated, per-item build log — every
shipped feature there is marked ✅ with the reasoning behind what was and wasn't built. Don't
duplicate that list here; it drifts. This file's job is to explain *how* things work, not to
re-track *whether* they're done.

## Planned / Todo

See **[ROADMAP.md](./ROADMAP.md)** for the full, current gap analysis against commercial rental
platforms. As of this file's last edit, the genuinely open items are: German + Russian
translations, rate management (day-of-week rates, blackout dates, per-location pricing), inbound
OTA booking ingestion, deposit pre-authorization/release, and inter-branch transfer tracking for
one-way rentals — everything else in prior versions of this list has since shipped.
